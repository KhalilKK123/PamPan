...
A physical phone is the sole runtime, connected-test, and TalkBack surface. I1B evidence carries to the exact I1E state through verified byte identity of all 20 protected Android behavior files; this is equivalence-based acceptance, not a retest. I2A is approved / active for the exact offline pantry contract preview in `docs/planning/active/pampan-native-rewrite-i2a.html`; because it changes protected Android behavior, it requires the fresh exact-state phone gate recorded there. I1C remains failed / BLOCKED historical evidence; I1D remains complete / PASS for Diagnostic PASS only. Do not mark the full rewrite complete: the I2 remainder and I3–I9 remain inactive and require fresh approval.

Android is the only active native application. Preserve `apps/ios/` as dormant; do not implement, build, test, verify, release, delete, or rewrite iOS work unless Rein gives fresh explicit approval. CI and validators must remain Android-only. Implement only the approved I2A contract preview; the I2 remainder and I3–I9 remain inactive and require fresh approval. Do not add product behavior beyond I2A, choose beyond the approved target stack, invent server contracts, resolve historical blockers without evidence, or change `pam_pan/` from `0512f84`.
