...
> **Repository state:** `PAMPAN-NATIVE-REWRITE` / I1A is implemented, independently verified and reviewed, and **complete / PASS** for state `I1A@0512f84/worktree-2d120810/index-e3b0c442/untracked-37221eae`. I1B is approved / not started for physical-device connected tests, offline launch, and user-observed TalkBack evidence. Android is the only active native application; `apps/ios/` is preserved dormant and excluded from implementation, build, test, CI, verification, and release gates unless fresh approval reactivates it. The larger native rewrite I1 remains **BLOCKED / pending verification**, not complete or PASS; even a local I1B PASS leaves successful actual-CI and emulator evidence for a separate increment. The legacy Flutter package remains unchanged from `0512f84`.
>
...
- Approved I1B plan: [physical-device verification preparation and evidence](docs/planning/active/pampan-native-rewrite-i1b.html)
