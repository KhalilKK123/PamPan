...
> **Repository state:** `PAMPAN-NATIVE-REWRITE` / I1E is the sole active successor and is **approved / pending implementation**. The revised acceptance policy makes GitHub Actions static-only and reserves runtime launch, connected testing, and TalkBack verification for an explicitly approved physical-phone gate. Completed physical-device I1B evidence may carry forward only through verified byte identity of every behavior-affecting Android production/test/manifest/Gradle/wrapper/dependency/lock/verification-metadata input; this is equivalence-based acceptance, not a retest, and later drift requires a freshly approved phone gate. I1C is preserved as **failed / BLOCKED historical evidence**; I1D remains **complete / PASS for Diagnostic PASS only**. Hosted-emulator verification is intentionally withdrawn and superseded, never retroactively passed. The larger I1 remains **BLOCKED / pending verification** until I1E implementation, one exact successful static-only GitHub Actions run, protected-byte equivalence, and configured final verification/review all pass. Android remains the only active native application; `apps/ios/` is dormant, `pam_pan/` remains unchanged from `0512f84`, and I2–I9 remain inactive.
>
...
- Active I1E plan: [static-only CI acceptance transition and hosted-emulator removal](docs/planning/active/pampan-native-rewrite-i1e.html)
