...
> **Repository state:** `PAMPAN-NATIVE-REWRITE` / I1A, I1B-R1, and physical-device I1B are independently verified/reviewed and **complete / PASS** only for their recorded exact states. I1B passed strict connected instrumentation, offline launch, and all approved manual foundation/TalkBack checks on exactly one user-authorized wired Android device; durable evidence is sanitized and identifier-free. I1C is **BLOCKED / failed single authorized run**: actual-CI and emulator evidence are FAIL, and both configured final gates returned FINAL FAIL for the exact attempt state. I1D is approved / not started for a sanitized diagnosis-first `-accel auto` recovery probe with distinct Recovery PASS and Diagnostic PASS outcomes. Android is the only active native application; `apps/ios/` is preserved dormant and excluded from implementation, build, test, CI, verification, and release gates unless fresh approval reactivates it. The larger native rewrite I1 remains **BLOCKED / pending verification** until genuine successful Android emulator evidence and a successful actual GitHub Actions run are recorded. The legacy Flutter package remains unchanged from `0512f84`, and I2–I9 remain inactive pending fresh approval.
>
...
- Active I1D plan: [sanitized emulator launch diagnosis with auto-mode recovery probe](docs/planning/active/pampan-native-rewrite-i1d.html)
